...
async def _section_goals(db) -> list[str]:
    ...
    reps = await db.execute_fetchall("SELECT * FROM goal_reps ORDER BY due_date, id")
    reps_by_goal: dict[int, list[dict]] = {}
    for rep in reps:
        reps_by_goal.setdefault(rep["goal_id"], []).append(dict(rep))

    lines = ["## Goals", "", "> Generated by Virgil. Edit goals and execution reps in Virgil, not in this file.", ""]
    for a in areas:
        a = dict(a)
        ...
        for horizon, h_label in horizon_labels.items():
            ...
            if area_goals:
                ...
                for g in area_goals:
                    window = ""
                    if g.get("start_date") or g.get("end_date"):
                        window = f" ({g.get('start_date') or 'open'} to {g.get('end_date') or 'open'})"
                    lines.append(f"- [{g.get('status', 'active')}] {g['content']}{window}")
                    for rep in reps_by_goal.get(g["id"], []):
                        lines.append(f"  - [{rep['status']}] {rep['content']} (due {rep['due_date']}, {rep['period']})")
                lines.append("")
    return lines
